...
function obtenerUsuario() {
    ...
    while(
        usuario===null|| usuario===""||usuario.length<3
    )
...
function renderizarAlbumes(listado) {
    ...
    covers.innerHTML = ``;
    // si ponemos solo el igual
        //se van a ir remplazando y queadria solo el ultimo Va el +=, 
    listado.forEach((album) => {
        covers.innerHTML+=`
        <li data-id="${album.id}">
        <img src="${album.imagen}" alt="${album.nombre}">
        <p>${album.nombre}</p>
        <i id="${album.id}" class="fa fa-heart ${album.like}"></i>
    </li>`
    });
...
function mostrarDatosEnPerfil() {
    ...
    const cantDeAlbumes =document.querySelector("#cant-albums");
    const cantFav = document.querySelector("#cant-favoritos")

    let contador=0;
    let cantidadLikes;
    
    
    cantDeAlbumes.innerText=`${albumesFamosos.length}  albumes`
...
albumesFamosos.forEach((album)=>{
        ...
        cantidadLikes=contador
    } )
   cantFav.innerText=`${cantidadLikes} favoritos`
}
