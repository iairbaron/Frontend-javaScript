...
function recuperarDataStorage() {
  const datosJson = localStorage.getItem("user");
...
/* -------------------------------------------------------------------------- */
// Ahora vamos a crear elementos en el DOM dinamicamente y
//le asignaremos a esos elementos la escucha de eventos.
// ☝ La funcion debe ser ejecutada al final del evento Load.
// La idea es crear un boton para cerrar sesión. Entonce
//necesitamos cumplir los siguientes puntos:
...
// 4- Tenemos que agregar el botón en pantalla, adentro
// del div con la clase 'user', al final del mismo
// // 5- El botón debe reaccionar cuando se le hace click
// // 6- Mediante el click debe aparecer un cuadro
// de confirmación que pregunte: "¿Seguro desea cerrar sesión?"
// // 7- Si el usuario acepta debe borrar todo el storage y
//  redirigirlo a la pantalla de Login.

function botonCerrarSesion() {
  let button = document.createElement("button");
  button.classList.add("botonCerrarSesion");
  let user = document.querySelector(".user");
  user.appendChild(button);
  button.innerHTML = "<p>Cerrar sesion</p> " ;

  button.addEventListener("click",function(){
  const pregunta= confirm("Seguro que queres salir?")
  if(confirm){
    localStorage.clear()
